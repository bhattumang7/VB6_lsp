...
| store-with-conversion outside the currently-modeled type-offset range | **Reclassified 2026-07-20: unreachable from the CURRENT front-end construction, same root cause as 0x61/0x0f/0x72.** The `0x8000` flag this gate checks comes from the assignment node's OWN `w[1]`, a property of how the node was built — `lower_udt_field_store` (the only current constructor of a UDT-field-store assignment node) always builds it via `NodeArena::node(0x2c, 0, 0, 0, 0, 0)`, which leaves `w[1]` at zero; the function never sets it. So `f_flags` can never carry `0x8000` for any assignment this front end currently lowers — 3 probe attempts (Byte-literal, Long→Byte, Long→Date field stores) confirmed this empirically (none reached the gate) before the root cause was traced. NOT a "capture the right type combo" problem — needs `lower_udt_field_store` (or a future construct) to set this flag under whatever condition the real `EbEmitAssignmentStmt` does, which requires tracing that C function first (not attempted this pass). |
