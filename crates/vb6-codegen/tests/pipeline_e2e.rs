...
// ── Owned-temp concatenation (a runtime-string operand) ─────────────────────
//
// When a `&` operand is a freshly-produced runtime-string result, the chain uses
// the cleanup-aware concat opcode (0xfb 0xef) over BSTR addresses: each operand is
// materialized to an address — an rtc result via its temp (0x04), a plain var via
// value-load + copy (0x6c…0x46), a literal via 0x3a — and the owned temps (rtc
// results, then concat results) are freed together at the end (0x35 / 0x36). All rtc
// operand temps are reserved before the materialization/concat temps.

#[test]
fn e2e_owned_concat_rtc_and_literal() {
    // s = UCase(t) & "!": UCase into result temp, "!" loaded via 0x3a (reference
    // index 1, after the rtc call took index 0), concat (fb ef), free both temps.
    assert_eq!(
        conv("Dim t As String, s As String", "s = UCase(t) & \"!\""),
        &[0x04, 0x78, 0xff, 0x4d, 0x64, 0xff, 0x08, 0x40, 0x04, 0x54, 0xff,
          0x0a, 0x00, 0x00, 0x08, 0x00, 0x04, 0x54, 0xff, 0x3a, 0x44, 0xff, 0x01, 0x00,
          0xfb, 0xef, 0x34, 0xff, 0x60, 0x31, 0x74, 0xff, 0x36, 0x04, 0x00, 0x54, 0xff,
          0x34, 0xff]
    );
}

#[test]
fn e2e_owned_concat_literal_first() {
    // s = "!" & UCase(t): the literal (reference index 0) precedes the rtc call
    // (index 1), but the rtc temps are still reserved first (higher addresses).
    assert_eq!(
        conv("Dim t As String, s As String", "s = \"!\" & UCase(t)"),
        &[0x3a, 0x44, 0xff, 0x00, 0x00, 0x04, 0x78, 0xff, 0x4d, 0x64, 0xff, 0x08, 0x40,
          0x04, 0x54, 0xff, 0x0a, 0x01, 0x00, 0x08, 0x00, 0x04, 0x54, 0xff,
          0xfb, 0xef, 0x34, 0xff, 0x60, 0x31, 0x74, 0xff, 0x36, 0x04, 0x00, 0x54, 0xff,
          0x34, 0xff]
    );
}
...
#[test]
fn e2e_owned_concat_with_plain_var() {
    // s = a & UCase(t): plain var a is value-loaded and copied to a temp (0x46); the
    // rtc temps are reserved ahead of a's copy temp (a-copy 0x40 < UCase temps).
    assert_eq!(
        conv("Dim t As String, a As String, s As String", "s = a & UCase(t)"),
        &[0x6c, 0x74, 0xff, 0x46, 0x40, 0xff, 0x04, 0x78, 0xff, 0x4d, 0x60, 0xff,
          0x08, 0x40, 0x04, 0x50, 0xff, 0x0a, 0x00, 0x00, 0x08, 0x00, 0x04, 0x50, 0xff,
          0xfb, 0xef, 0x30, 0xff, 0x60, 0x31, 0x70, 0xff, 0x36, 0x04, 0x00, 0x50, 0xff,
          0x30, 0xff]
    );
}

#[test]
fn e2e_owned_concat_two_rtc_results() {
    // s = UCase(t) & LCase(u): both rtc temps reserved first (UCase 0x50, LCase
    // 0x30), then the concat result (0x20); all three owned temps freed (0x36).
    assert_eq!(
        conv("Dim t As String, u As String, s As String", "s = UCase(t) & LCase(u)"),
        &[0x04, 0x78, 0xff, 0x4d, 0x60, 0xff, 0x08, 0x40, 0x04, 0x50, 0xff,
          0x0a, 0x00, 0x00, 0x08, 0x00, 0x04, 0x50, 0xff, 0x04, 0x74, 0xff, 0x4d, 0x40,
          0xff, 0x08, 0x40, 0x04, 0x30, 0xff, 0x0a, 0x01, 0x00, 0x08, 0x00, 0x04, 0x30,
          0xff, 0xfb, 0xef, 0x20, 0xff, 0x60, 0x31, 0x70, 0xff, 0x36, 0x06, 0x00, 0x50,
          0xff, 0x30, 0xff, 0x20, 0xff]
    );
}

#[test]
fn e2e_owned_concat_chain_with_vars() {
    // s = UCase(t) & b & c: rtc temps first, then each plain var copy and concat in
    // emission order; two fb ef ops, three owned temps freed.
    assert_eq!(
        conv("Dim t As String, b As String, c As String, s As String", "s = UCase(t) & b & c"),
        &[0x04, 0x78, 0xff, 0x4d, 0x5c, 0xff, 0x08, 0x40, 0x04, 0x4c, 0xff,
          0x0a, 0x00, 0x00, 0x08, 0x00, 0x04, 0x4c, 0xff, 0x6c, 0x74, 0xff, 0x46, 0x3c,
          0xff, 0xfb, 0xef, 0x2c, 0xff, 0x6c, 0x70, 0xff, 0x46, 0x1c, 0xff, 0xfb, 0xef,
          0x0c, 0xff, 0x60, 0x31, 0x6c, 0xff, 0x36, 0x06, 0x00, 0x4c, 0xff, 0x2c, 0xff,
          0x0c, 0xff]
    );
}

#[test]
fn e2e_owned_concat_multiarg_rtc() {
    // s = Left(t, n) & x: a multi-argument rtc (Left, arg-bytes 0x0c) as the owned
    // operand, concatenated with a plain var.
    assert_eq!(
        conv("Dim t As String, n As Long, s As String, x As String", "s = Left(t, n) & x"),
        &[0x6c, 0x74, 0xff, 0x04, 0x78, 0xff, 0x4d, 0x5c, 0xff, 0x08, 0x40, 0x04, 0x4c,
          0xff, 0x0a, 0x00, 0x00, 0x0c, 0x00, 0x04, 0x4c, 0xff, 0x6c, 0x6c, 0xff, 0x46,
          0x3c, 0xff, 0xfb, 0xef, 0x2c, 0xff, 0x60, 0x31, 0x70, 0xff, 0x36, 0x04, 0x00,
          0x4c, 0xff, 0x2c, 0xff]
    );
}

// ── Owned-temp Len (Len of a runtime-string result) ─────────────────────────
//
// `Len(UCase(t))` produces the rtc result into a temp, then takes its length with
// the release-aware Len opcode (0xfb 0xeb <scratch>) followed by 0xfc 0x22, storing
// the Long; the rtc result temp is freed (0x35). The Len scratch temp is not freed.

#[test]
fn e2e_owned_len_single_arg_rtc() {
    assert_eq!(
        conv("Dim t As String, r As Long", "r = Len(UCase(t))"),
        &[0x04, 0x78, 0xff, 0x4d, 0x64, 0xff, 0x08, 0x40, 0x04, 0x54, 0xff,
          0x0a, 0x00, 0x00, 0x08, 0x00, 0x04, 0x54, 0xff, 0xfb, 0xeb, 0x44, 0xff,
          0xfc, 0x22, 0x71, 0x74, 0xff, 0x35, 0x54, 0xff]
    );
}

#[test]
fn e2e_owned_len_multiarg_rtc() {
    // Len(Left(t, n)): the rtc is a multi-argument call (arg-bytes 0x0c); the owned
    // Len machinery is unchanged.
    assert_eq!(
        conv("Dim t As String, n As Long, r As Long", "r = Len(Left(t, n))"),
        &[0x6c, 0x74, 0xff, 0x04, 0x78, 0xff, 0x4d, 0x60, 0xff, 0x08, 0x40, 0x04, 0x50,
          0xff, 0x0a, 0x00, 0x00, 0x0c, 0x00, 0x04, 0x50, 0xff, 0xfb, 0xeb, 0x40, 0xff,
          0xfc, 0x22, 0x71, 0x70, 0xff, 0x35, 0x50, 0xff]
    );
}

// ── Owned-temp string comparison (a runtime-string operand) ─────────────────
//
// A String relational comparison with a runtime-string operand uses the
// compare-into-temp opcode (0xfb <op>, one less than the plain second byte) wrapped
// by the owned markers 0x5d (after the non-owned operand) and 0x55. Only the rtc
// result temps are freed; the compare temp is not.

#[test]
fn e2e_owned_compare_rtc_vs_literal() {
    // r = (UCase(t) = "X"): UCase result and the literal materialized; 0x5d follows
    // the non-owned literal; compare-into-temp (fb 2f), finalize (0x55), store, free.
    assert_eq!(
        conv("Dim t As String, r As Integer", "r = (UCase(t) = \"X\")"),
        &[0x04, 0x78, 0xff, 0x4d, 0x64, 0xff, 0x08, 0x40, 0x04, 0x54, 0xff,
          0x0a, 0x00, 0x00, 0x08, 0x00, 0x04, 0x54, 0xff, 0x3a, 0x44, 0xff, 0x01, 0x00,
          0x5d, 0xfb, 0x2f, 0x34, 0xff, 0x55, 0x70, 0x76, 0xff, 0x35, 0x54, 0xff]
    );
}

#[test]
fn e2e_owned_compare_all_operators() {
    // The compare-into-temp second byte is the plain string-compare byte minus one.
    let decl = "Dim t As String, b As String, r As Integer";
    for (stmt, op) in [
        ("r = (UCase(t) = b)", 0x2fu8),
        ("r = (UCase(t) <> b)", 0x3c),
        ("r = (UCase(t) < b)", 0x63),
        ...
        assert_eq!(
            conv(decl, stmt),
            &[0x04, 0x78, 0xff, 0x4d, 0x60, 0xff, 0x08, 0x40, 0x04, 0x50, 0xff,
              0x0a, 0x00, 0x00, 0x08, 0x00, 0x04, 0x50, 0xff, 0x6c, 0x74, 0xff, 0x46,
              0x40, 0xff, 0x5d, 0xfb, op, 0x30, 0xff, 0x55, 0x70, 0x72, 0xff, 0x35,
              0x50, 0xff],
            "owned compare {stmt}"
        );
    }
...
#[test]
fn e2e_owned_compare_rtc_on_right() {
    // r = (b = UCase(t)): the owned operand is on the right; 0x5d follows the
    // non-owned left operand (b) instead.
    assert_eq!(
        conv("Dim t As String, b As String, r As Integer", "r = (b = UCase(t))"),
        &[0x6c, 0x74, 0xff, 0x46, 0x40, 0xff, 0x5d, 0x04, 0x78, 0xff, 0x4d, 0x60, 0xff,
          0x08, 0x40, 0x04, 0x50, 0xff, 0x0a, 0x00, 0x00, 0x08, 0x00, 0x04, 0x50, 0xff,
          0xfb, 0x2f, 0x30, 0xff, 0x55, 0x70, 0x72, 0xff, 0x35, 0x50, 0xff]
    );
}

#[test]
fn e2e_owned_compare_both_rtc() {
    // r = (UCase(t) = LCase(u)): both operands owned → no 0x5d marker; both rtc
    // result temps freed (0x36).
    assert_eq!(
        conv("Dim t As String, u As String, r As Integer", "r = (UCase(t) = LCase(u))"),
        &[0x04, 0x78, 0xff, 0x4d, 0x60, 0xff, 0x08, 0x40, 0x04, 0x50, 0xff,
          0x0a, 0x00, 0x00, 0x08, 0x00, 0x04, 0x50, 0xff, 0x04, 0x74, 0xff, 0x4d, 0x40,
          0xff, 0x08, 0x40, 0x04, 0x30, 0xff, 0x0a, 0x01, 0x00, 0x08, 0x00, 0x04, 0x30,
          0xff, 0xfb, 0x2f, 0x20, 0xff, 0x55, 0x70, 0x72, 0xff, 0x36, 0x04, 0x00, 0x50,
          0xff, 0x30, 0xff]
    );
}
