...
// ── emit_call (by-reference / method call site) ──────────────────────────────
//
// emit_call ports the call-site emission consuming a resolved CallDescriptor.
// The binder (symbol-table model) supplies kind/byref/flags/member_id and the
// callee/arg nodes; the emission itself is exact given the descriptor. Here the
// callee is a global-load stand-in (GL0) so its bytes are known.

#[test]
fn emit_call_method_emits_callee_then_opcode() {
    // kind 8, ByRef, flags 0, type region 0x80000 (not 0x20000/0x120000), type
    // tag 8 (not Variant-group): call_op = type_code(8,ref) | map3(off[8]=2)
    //   = 0x320 | 2 = 0x322. emit_mode 0 -> no member word.
    let mut a = NodeArena::new();
    let callee = global_long_load(&mut a, 0);
    let desc = crate::emit::CallDescriptor {
        kind: 8,
        byref: 1,
        flags: 0,
        node_word0: 0x8_0000,
    ...
    e.emit_call(&desc, 0);
    let mut expected = GL0.to_vec();
    expected.extend(v2(0x322));
    assert_eq!(e.into_bytes(), expected.as_slice());
}

#[test]
fn emit_call_dispatch_emits_member_word() {
    // flags bit 0x8000 set -> emit_mode 1: call_op = map_call_type_code(0x320) =
    // 0x34f, and the member-id word follows the opcode.
    let mut a = NodeArena::new();
    let callee = global_long_load(&mut a, 0);
    let desc = crate::emit::CallDescriptor {
        kind: 8,
    ...
    expected.extend_from_slice(&[0x34, 0x12]); // member id LE
    assert_eq!(e.into_bytes(), expected.as_slice());
}

#[test]
fn emit_call_emits_args_before_callee() {
    // A forward arg list (0x37 node) is emitted before the callee, in order.
    let mut a = NodeArena::new();
    let arg = global_long_load(&mut a, 4); // GL4
    let arg_list = a.alloc(NodeArena::node(0x37, 0, arg.0, 0, 0, 0));
    let callee = global_long_load(&mut a, 0); // GL0
    let desc = crate::emit::CallDescriptor {
        kind: 8,
        byref: 1,
        flags: 0,
    ...
    let mut e = Emitter::new(&a);
    e.emit_call(&desc, 0);
    let mut expected = GL4.to_vec();
    expected.extend_from_slice(&GL0);
    expected.extend(v2(0x322));
    assert_eq!(e.into_bytes(), expected.as_slice());
}
